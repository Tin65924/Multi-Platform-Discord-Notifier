...
Security model:
  ...
  - Always re-encoded (strips EXIF, normalizes orientation, kills
    polyglot/exploit payloads hiding after image data).
  - Served from Postgres (survives Render free restarts), never disk.
"""
import io
import logging
...
def process_upload(raw: bytes) -> tuple[bytes, str]:
    ...
    try:
        ...
        pass
    img = ImageOps.exif_transpose(img)  # fix phone rotation, strip orientation tag
